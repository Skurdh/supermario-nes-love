...
local RedKoopa = Ennemy:extend('Red Koopa')
...
function RedKoopa:new(x, y, linked)
    ...
    self.behaviours = {
        ...
        fall = function(dt)
            ...
            if (collX) then 
                if (collX == 'left') then self.sx = 1
                elseif (collX == 'right') then self.sx = -1 end 
            end
...
function RedKoopa:hit(target, hitType, arg)
...
function RedKoopa:coillisionVoid()
    local xPos, yPos
    ...
    if (self.dx ~= 0 and scene:getMap():getTile(scene:getMap():pixelToTile(xPos, yPos)) == 0) then
        if (self.dx < 0) then self.dx = -self.speed self.sx = -1
        elseif (self.dx > 0) then self.dx = self.speed self.sx = 1 end
    end  
end

function RedKoopa:update(dt)
